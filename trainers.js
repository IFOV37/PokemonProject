...
module.exports = function(){
    ...
    function getTrainers(res, mysql, context, complete){
    	mysql.pool.query("SELECT Trainers.id, Trainers.name AS name, catchphrase, p.name AS pokemon FROM Trainers INNER JOIN Pokemon p ON p.trainerID = Trainers.id", function(error, results, fields){
    		if(error){
    ...
    function getTrainersBadges(res, mysql, context, id, complete){
        var sql = "SELECT b.id, b.name, tb.trainerID FROM Badges b INNER JOIN Trainer_Badge tb ON b.ID = tb.badgeID WHERE tb.trainerID = ?";
        var inserts = [id];
        mysql.pool.query(sql, inserts, function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
